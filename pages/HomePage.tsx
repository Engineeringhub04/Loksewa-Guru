...
const SideMenu: React.FC<{ 
    ...
    const MenuItem: React.FC<{service: ServiceDocument}> = ({ service }) => {
        ...
        const handleClick = () => {
            ...
            const { login: loginRequired, subscription: subscriptionRequired } = subscriptionLocks[service.key] || {};
            ...
            >
                ...
                <div className="p-2 border-t dark:border-gray-700 flex-shrink-0">
                    <div className="relative" ref={profileMenuRef}>
                        ...
                        <button 
                            onClick={() => isLoggedIn ? setIsProfileMenuOpen(p => !p) : navigate('/login', { state: { from: location } })}
...
const AdditionalFeaturesSection: React.FC<{ features: AdditionalFeatureData[] }> = ({ features }) => {
    ...
    const handleClick = (feature: AdditionalFeatureData) => {
        const { login: loginRequired, subscription: subscriptionRequired } = subscriptionLocks[feature.key] || {};
        ...
        <>
            <div ref={scrollRef} className={`p-4 ${animationClasses}`}>
                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md text-center">
                    ...
                    <button onClick={handleUploadClick} className="px-6 py-3 bg-purple-600 text-white font-semibold rounded-full hover:bg-purple-700 transition-colors shadow-lg">
                        Upload Notes
                    </button>
                </div>
            </div>
...
const HomePage: React.FC = () => {
  ...
  const seenToastIds = useRef(new Set<string>());
    ...
    useEffect(() => {
        if (isMenuOpen) {
            document.body.classList.add('sidemenu-is-open');
        } else {
            document.body.classList.remove('sidemenu-is-open');
        }
        return () => {
            document.body.classList.remove('sidemenu-is-open');
        };
