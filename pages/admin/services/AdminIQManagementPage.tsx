...
const AdminIQManagementPage: React.FC = () => {
    ...
    const MAIN_CAT_KEY = 'general'; // Assumption: IQ quizzes belong to a 'general' main category
    const SUB_CAT_KEY = 'iq';
    ...
    const fetchQuizzes = useCallback(async () => {
        ...
        try {
            ...
            console.error("Error fetching IQ quizzes:", err);
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <header className="flex justify-between items-center mb-6 border-b dark:border-gray-700 pb-4">
                <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Manage IQ Quizzes</h1>
                <button onClick={handleCreateNew} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                    <PlusCircleIcon className="h-5 w-5" /> Add New IQ Quiz
                </button>
            </header>
            ...
            {!loading && !error && (
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {quizzes.length === 0 ? (
                                <tr><td colSpan={4} className="text-center py-8 text-gray-500">No IQ quizzes found.</td></tr>
                            ) : (
                                quizzes.map(quiz => (
...
export default AdminIQManagementPage;